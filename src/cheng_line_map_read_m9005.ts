...
var initChengLineMapReadModule=defineModuleInitializer(()=>{
  ...
  chengLineMapReadInputSchema=zodSchema.strictObject({
    file:zodSchema.string().describe(".cheng source path or an existing cheng_line_map text file."),
  ...
  ChengLineMapReadTool=createChengTextTool({
    ...
    description:"Return deterministic top-level function signature/body/end spans from the current .cheng bytes, or parse an explicitly supplied canonical Cheng line-map file.",
