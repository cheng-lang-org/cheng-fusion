// cheng_crash_triage: {binary,args,env,primaryObject} 结构化输入模式 —
...
async function testCFixtureLiveCrash() {
  ...
  try {
    ...
    assertTrue(parsed.schema === "cheng_crash_triage_live", `schema 精确, 实得 ${parsed.schema}`);
...
async function testStderrModeRegression() {
  ...
  try {
    ...
    assertTrue(parsed.schema === "cheng_crash_triage", `stderr schema 精确, 实得 ${parsed.schema}`);
...
async function testGen2mRealDriverCrashIfPresent() {
  ...
  try {
    ...
    assertTrue(parsed.stopClass === "SIGSEGV", `stopClass 按 Darwin mach 异常映射分类, 实得 ${parsed.stopClass}`);
  } finally {
    mcp.kill();
    rmSync(dir, {recursive: true, force: true});
  }
...
async function main() {
  ...
  console.log("item6 crash triage: PASS");
}

main().catch((error) => {
  console.error("item6 crash triage: FAIL", error);
  process.exit(1);
});
